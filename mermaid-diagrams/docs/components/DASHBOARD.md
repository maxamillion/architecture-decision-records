# Dashboard (Mermaid Version)

> This document provides the same information as [`documentation/components/dashboard/README.md`](../../../documentation/components/dashboard/README.md) but uses interactive Mermaid diagrams.
...
- Every request crosses the OAuth Proxy line on the way to getting data, and thus is authenticated through that mechanism per request (using bearer tokens on each request)
...
[View Detailed Diagram](../../dashboard-feature-flags.md)

The Dashboard implements a feature flag system with 2-minute cache refresh for dynamic feature control. The system integrates with DSC (DataScienceCluster) and DSCI (DSCInitialization) resources to determine feature availability.
...
Read more about the specifics in [Configuring the Dashboard](../../../documentation/components/dashboard/configuringDashboard.md)

### Dashboard Feature Details

Most features in the Dashboard are documented by other component's documentation. However, there are details about connectivity & specific Dashboard features that are documented in the [Features](../../../documentation/components/dashboard/features/README.md) section.
...
Read more details about our storage mechanisms in [Dashboard Storage Mechanisms](../../../documentation/components/dashboard/dashboardStorage.md)
...
1. **Cluster Admins** (`cluster-admin` role granted at the cluster level)
   ...
   - It is expected that this type of user is extremely small in number due to the implicit security concerns it would mean to the cluster
...
2. **Product Admins**
   ...
   - More information can be found in the admins section
...
3. **Project Users**
   ...
   - There are 3 variants to this:
     - **Admin**: can change permissions (invite/remove people from the project) & modify the name/description of the project
     - **Contributor** (aka Edit): Can do everything but what is explicitly noted for Admins
     - **View**: We do not support this state -- but effectively they can change nothing and also cannot see secrets (Connections)
...
> Note: You can read more about `opendatahub.io/dashboard` in [Dashboard K8s Labels & Annotations](../../../documentation/components/dashboard/k8sLabelsAndAnnotations.md#opendatahubiodashboard)
...
These resources help provide information of your accelerators on your cluster and gives the admin the ability to configure or disable them as they see fit. In turn, the user can get access to "profiles" of how accelerators work.
...
- **Full Documentation**: [documentation/components/dashboard/README.md](../../../documentation/components/dashboard/README.md)
- **Feature Flag Diagram**: [dashboard-feature-flags.md](../../dashboard-feature-flags.md)
